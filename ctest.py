...
class FinancialTermAnnotator:
    ...
    def initialize(self):
        """벡터 DB와 QA 체인을 초기화합니다."""
        try:
            st.info("🔧 전문용어 주석 시스템 초기화 중...")
            
            # 1. 한국은행 PDF 문서 로딩 (선택적)
            pdf_docs = []
            try:
                with st.spinner("PDF 문서 로딩 중..."):
                    pdf_loader = PyPDFLoader("경제금융용어 700선.pdf")
                    pdf_docs = pdf_loader.load()
                st.success("✅ PDF 문서 로딩 완료")
            except FileNotFoundError:
                st.warning("⚠️ '경제금융용어 700선.pdf' 파일을 찾을 수 없습니다. QA 데이터셋만 사용합니다.")
            except Exception as e:
                st.warning(f"⚠️ PDF 로딩 중 오류: {e}. QA 데이터셋만 사용합니다.")
            
            # 2. QA 데이터셋 로딩
            with st.spinner("QA 데이터셋 로딩 중..."):
                qa_dataset = load_dataset("coorung/Kor-financial-qa-7K", split="train")
            
            # 3. QA 데이터셋을 Document 리스트로 변환
            ...
            with st.spinner("문서 분할 중..."):
                ...
                if not all_docs:
                    raise Exception("로딩된 문서가 없습니다.")
                splitter = CharacterTextSplitter(chunk_size=642, chunk_overlap=50)
                split_docs = splitter.split_documents(all_docs)
            
            # 5. 임베딩 모델 및 벡터 DB 구축
            with st.spinner("벡터 DB 구축 중..."):
                embedding_model = HuggingFaceEmbeddings(model_name="snunlp/KR-SBERT-V40K-klueNLI-augSTS")
                self.vector_db = Chroma.from_documents(split_docs, embedding_model, persist_directory="./chroma_db")
            ...
            st.success("✅ 전문용어 주석 시스템 초기화 완료!")
            
        except Exception as e:
            st.error(f"❌ 전문용어 주석 시스템 초기화 실패: {e}")
            self.is_initialized = False
    
    def extract_financial_terms(self, text: str) -> list:
        """텍스트에서 금융 전문용어를 추출합니다."""
        pattern = r'\b[A-Z]{2,6}\b|[가-힣]{2,10}'
        terms = re.findall(pattern, text)
        stop_words = {"분석", "시장", "경제", "주식", "전망", "보고서", "뉴스", "투자", "금융"}
        return list(set(t for t in terms if len(t) > 1 and t not in stop_words))
    
    def explain_terms(self, text: str) -> str:
        """텍스트의 전문용어에 주석을 추가합니다."""
        if not self.is_initialized:
            return text + "\n\n⚠️ 전문용어 주석 시스템이 초기화되지 않았습니다."
        ...
        for term in terms:
            if term.lower() in seen_terms:
                continue
            try:
                result = self.qa_chain.invoke({"query": term})
                answer = result["result"].strip()
                if answer and len(answer) > 10:
                    explained_terms.append(f"- **{term}**: {answer}")
                    seen_terms.add(term.lower())
            except Exception:
                continue

        if explained_terms:
            explanation_block = "\n\n---\n\n📌 **전문용어 해설**\n" + "\n".join(explained_terms)
            return text + explanation_block
        else:
            return text + "\n\n📌 전문용어 설명이 없습니다."

# 전역 전문용어 주석 시스템 인스턴스
term_annotator = FinancialTermAnnotator()
                ...
                for event in events:
                    for k, v in event.items():
                        if k == 'extract_keyword':
                            ...
                            # 전문용어 주석이 포함된 답변 표시
                            if "📌 **전문용어 해설**" in v['answer']:
                                # 전문용어 해설이 있는 경우 분리하여 표시
                                parts = v['answer'].split("📌 **전문용어 해설**")
                                main_report = parts[0]
                                term_explanations = "📌 **전문용어 해설**" + parts[1] if len(parts) > 1 else ""
...
<div style='text-align: center; color: #666;'>
    <p>Powered by Streamlit + LangGraph + Naver ClovaX + EconoTimes</p>
